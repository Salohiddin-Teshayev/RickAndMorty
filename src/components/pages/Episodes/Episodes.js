...
const Episodes = () => {
    ...
    const [episodeCharactersData, setEpisodeCharactersData] = useState(JSON.parse(localStorage.getItem('episodeData') || '[]'));
    const [character, setCharacter] = useState({});
    let api = `https://rickandmortyapi.com/api/episode?page=${page}`;
    ...
    useEffect(() => {
        setEpisodeCharactersData([]);
        if(!data) {
            setData(JSON.parse(localStorage.getItem('data') || []));
        }
        const episodeCharacters = data.find(item => Number(item.id) === Number(episode));
        episodeCharacters?.characters?.forEach((link) => {
            fetch(link)
            .then(res => res.json())
            .then(data => setEpisodeCharactersData((prev) => [...prev, data]));
        });
        
        localStorage.setItem('episodeData', JSON.stringify(episodeCharactersData || []));
    ...
    <>
        ...
        <div className='container mx-auto px-6'>
                <h1 className='my-6 mb-9 text-4xl text-center text-slate-600'>Episodes</h1>
                <strong className='block mb-8 font-mono font-semibold text-3xl text-center'>Episode - {episode}: <span className='text-lime-600'>{data[Number(episode) - 1]?.name}</span></strong>
                <div className='flex gap-10'>
                    <div className='w-3/12 flex flex-col items-center'>
                        <h2 className='mb-8 text text-3xl text-slate-700 text-center'>Choose episode</h2>
                        <form onSubmit={(evt) => evt.preventDefault()} className='flex w-full'>
                            <select onChange={(e) => setEpisode(e.target.value)} className='w-full p-3 border-2 border-slate-500 rounded-md text-slate-600 shadow-md text-lg' name='episode'>
                                {
                                    data.length ? data.map(item => <option key={item.id} value={item.id}>Episode {item.id} - {item.name}</option>) : ''
                                }
                            </select>
                        </form>
                    </div>
                    <div className='w-8/12 flex flex-col items-center'>
                         <ul className='list-unstyled w-full flex flex-wrap gap-y-4 justify-center mb-5'>
                        {
                            ...
                            episodeCharactersData?.map((item,index) => {
                                return (
